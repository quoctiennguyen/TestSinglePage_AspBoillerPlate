...
namespace Trainer.Tests
{
    public abstract class TrainerTestBase : AbpIntegratedTestBase<TrainerTestModule>
    {
        ...
        protected void UsingDbContext(Action<TrainerDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<TrainerDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TrainerDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TrainerDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TrainerDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TrainerDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<TrainerDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TrainerDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TrainerDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TrainerDbContext, Task<T>> func)
        {
            T result;
